...
mod tests {
    ...
    /// `list_transactions` rejects malformed dates before querying.
    #[tokio::test]
    async fn list_transactions_rejects_malformed_date() {
        let server = in_memory_server().await;
    ...
    /// `get_transaction` rejects invalid UUIDs.
    #[tokio::test]
    async fn get_transaction_rejects_invalid_uuid() {
        let server = in_memory_server().await;

        let result = server
            .get_transaction(Parameters(GetTransactionParams {
                transaction_id: "not-a-uuid".into(),
            }))
            .await;
    ...
    /// `aggregate_transactions` rejects unknown aggregation levels.
    #[tokio::test]
    async fn aggregate_transactions_rejects_unknown_level() {
        let server = in_memory_server().await;

        let result = server
            .aggregate_transactions(Parameters(AggregateTransactionsParams {
                start_date: None,
                end_date: None,
                aggregated_level: "yearly".into(),
                kind: None,
                categories: vec![],
                classifications: vec![],
            }))
    ...
    /// `pivot_expenses` rejects malformed dates.
    #[tokio::test]
    async fn pivot_expenses_rejects_malformed_date() {
        let server = in_memory_server().await;

        let result = server
            .pivot_expenses(Parameters(PivotExpensesParams {
                start_date: Some("not-a-date".into()),
                end_date: None,
                categories: vec![],
                classifications: vec![],
